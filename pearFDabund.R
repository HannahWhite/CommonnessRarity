################################################################################
### Sequential correlations of abundance-based functional diversity measures ###
################################################################################

#### Hannah White 05.05.2021
#### Adapted code so that uses relative % cover rather than presence/absence

### Need to select relevant data for scale want to work at
...
c2r.fdisabund <- array(data = NA, dim = c(475, n))
rownames(c2r.fdisabund) <- site.sp$plot.label

c2r.raoqabund <- array(data = NA, dim = c(475, n))
...
system.time(
  for (sp in 2:n) {
    ...
    out <- FD::dbFD(sub.traits, comm.alpha, calc.CWM = FALSE, w = w)
    
    out.fdis <- out$FDis
    out.raoq <- out$RaoQ
    
    c2r.fdisabund[names(out.fdis), sp] <- out.fdis
    c2r.raoqabund[names(out.raoq), sp] <- out.raoq
...
c2r.pear.fdisabund <- apply(c2r.fdisabund, 2, FUN = function(x2) cor(cbind(x2, c2r.fdisabund[,n]), method = 'pearson', use = 'complete.obs')[[2]])
c2r.pear.raoqabund <- apply(c2r.raoqabund, 2, FUN = function(x2) cor(cbind(x2, c2r.raoqabund[,n]), method = 'pearson', use = 'complete.obs')[[2]])

c2r.pearcorrs <- data.frame(c2r.pear.fdisabund, c2r.pear.raoqabund)
...
r2c.fdisabund <- array(data = NA, dim = c(475, n))
rownames(r2c.fdisabund) <- site.sp$plot.label

r2c.raoqabund <- array(data = NA, dim = c(475, n))
...
system.time(
  for (sp in 2:n) {
    ...
    r2c.fdisabund[names(out.fdis), sp] <- out.fdis
    r2c.raoqabund[names(out.raoq), sp] <- out.raoq
    
  }
) # system.time elapsed 378.20
...
### Pearsons
## Pearson correlations
r2c.pear.fdisabund <- apply(r2c.fdisabund, 2, FUN = function(x2) cor(cbind(x2, r2c.fdisabund[,n]), method = 'pearson', use = 'complete.obs')[[2]])
r2c.pear.raoqabund <- apply(r2c.raoqabund, 2, FUN = function(x2) cor(cbind(x2, r2c.raoqabund[,n]), method = 'pearson', use = 'complete.obs')[[2]])

r2c.pearcorrs <- data.frame(r2c.pear.fdisabund, r2c.pear.raoqabund)
...
pearFD.df <- data.frame(subassem, 
                        c2r.fdisabund = c2r.pearcorrs[,1], c2r.raoqabund = c2r.pearcorrs[,2],
                        r2c.fdisabund = r2c.pearcorrs[,1], r2c.raoqabund = r2c.pearcorrs[,2], 
...
## load randomisations
load('randomcorrs.c2rabund1m.RData')
load('randomcorrs.r2cabund1m.RData')

subassem <- pearFD1mabund$subassem
sum.pq.c2r <- pearFD1mabund$sum.pq.c2r
sum.pq.r2c <- pearFD1mabund$sum.pq.r2c


### FDis
# Subassemblage 

c2r.fdis.rand <- data.frame(subassem, fdisabund.corr)
c2r.fdis.rand <- melt(c2r.fdis.rand, id.vars="subassem")

r2c.fdis.rand <- data.frame(subassem, r2c.fdisabund.corr)
...
p.infofdis <- p.infofdis + geom_line(data = pearFD.df, size = 1.5,
                                     aes(x = sum.pq.c2r, y = c2r.fdisabund, col = 'gold1')) + 
  geom_rug(data = pearFD.df, col = 'gold1', aes(x = sum.pq.c2r))
p.infofdis <- p.infofdis + geom_line(data = pearFD.df, size = 1.5,
                                     aes(x = sum.pq.r2c, y = r2c.fdisabund, col = 'forestgreen')) + 
...
c2r.raoq.rand <- data.frame(subassem, raoqabund.corr)
c2r.raoq.rand <- melt(c2r.raoq.rand, id.vars="subassem")

r2c.raoq.rand <- data.frame(subassem, r2c.raoqabund.corr)
...
c2r.raoq.rand.pq <- data.frame(sum.pq.c2r, raoqabund.corr)
c2r.raoq.rand.pq <- melt(c2r.raoq.rand.pq, id.vars="sum.pq.c2r")

r2c.raoq.rand.pq <- data.frame(sum.pq.r2c, r2c.raoqabund.corr)
...
p.inforaoq <- p.inforaoq + geom_line(data = pearFD.df, size = 1.5,
                                     aes(x = sum.pq.c2r, y = c2r.raoqabund, col = 'gold1')) + 
  geom_rug(data = pearFD.df, col = 'gold1', aes(x = sum.pq.c2r))
p.inforaoq <- p.inforaoq + geom_line(data = pearFD.df, size = 1.5,
                                     aes(x = sum.pq.r2c, y = r2c.raoqabund, col = 'forestgreen')) + 
  geom_rug(data = pearFD.df, col = 'forestgreen', alpha = 0.7, aes(x = sum.pq.r2c))
