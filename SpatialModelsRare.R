###################################################################################
### Modeled relationship between proportion rare species in sites and diversity ###
###################################################################################

### Hannah White 05.05.2021

### Conditional autoregressive models of diversity measures as a function of the 
### proportion of the community made up of the 25% rarest species
...
#### Top 10 % of each
top4cm <- r2c4cm[1:(ceiling(length(r2c4cm)/4))]
top20cm <- r2c20cm[1:(ceiling(length(r2c20cm)/4))]
top1m <- r2c1m[1:(ceiling(length(r2c1m)/4))]
top5m <- r2c5m[1:(ceiling(length(r2c5m)/4))]

rare.prop <- function(x, y){
  sp.pres <- x[which(x==1)]
  rare <- length(which(names(sp.pres) %in% y))
  rich <- sum(x)
  prop <- rare/rich
  return(prop)
}

### Calculate proportion of each site that are 'rare' species
rare4cm <- apply(site.sp4cm[,5:62], 1, FUN = function(x) rare.prop(x, top4cm))
rare20cm <- apply(site.sp20cm[,5:90], 1, FUN = function(x) rare.prop(x, top20cm))
rare1m <- apply(site.sp1m[,5:119], 1, FUN = function(x) rare.prop(x, top1m))
rare5m <- apply(site.sp5m[,2:116], 1, FUN = function(x) rare.prop(x, top5m))
...
rarediv4cm <- data.frame(plot.label = site.sp4cm$plot.label, rare4cm, sr4cm, full.fd4cm, fdis4cm, raoq4cm, fdisabund4cm, raoqabund4cm)
rarediv20cm <- data.frame(plot.label = site.sp20cm$plot.label, rare20cm, sr20cm, full.fd20cm, fdis20cm, raoq20cm, fdisabund20cm, raoqabund20cm)
rarediv1m <- data.frame(plot.label = site.sp1m$plot.label, rare1m, sr1m, full.fd1m, fdis1m, raoq1m, fdisabund1m, raoqabund1m)
rarediv5m <- data.frame(plot.label = site.sp5m$Plot.ID, rare5m, sr5m, full.fd5m, fdis5m, raoq5m, fdisabund5m, raoqabund5m)
...
rarediv4cm <- merge(sites4cm, rarediv4cm, by = 'plot.label', all.y = TRUE)
rarediv20cm <- merge(sites20cm, rarediv20cm, by = 'plot.label', all.y = TRUE)
rarediv1m <- merge(sites1m, rarediv1m, by = 'plot.label', all.y = TRUE)
rarediv5m <- merge(sites5m, rarediv5m, by = 'plot.label', all.y = TRUE)


### Models
## SR
## neighbourhood matrix
coords4cm <- rarediv4cm[,2:3]
dists4cm <- as.matrix(dist(coords4cm, method = 'maximum'))
w4cm <- array(0, dim = c(475, 475))
w4cm[dists4cm <= 16 & dists4cm > 0] <- 1 # gets 5 x 5 grid

coords20cm <- rarediv20cm[,2:3]
...
set.seed(32)
mod.sr4cm <- S.CARleroux(log(sr4cm) ~ rare4cm, family = 'gaussian', data = rarediv4cm, W = w4cm, 
...
set.seed(32)
mod.sr20cm <- S.CARleroux(log(sr20cm) ~ rare20cm, family = 'gaussian', data = rarediv20cm, W = w20cm, 
                          burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                          rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.sr1m <- S.CARleroux(log(sr1m) ~ rare1m, family = 'gaussian', data = rarediv1m, W = w1m, 
                        burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                        rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.sr5m <- MCMCglmm(log(sr5m) ~ rare5m, family = 'gaussian', data = rarediv5m,
...
### P + G
set.seed(32)
mod.pg4cm <- S.CARleroux(full.fd4cm ~ rare4cm, family = 'gaussian', data = rarediv4cm, W = w4cm, 
                         burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                         rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.pg20cm <- S.CARleroux(full.fd20cm ~ rare20cm, family = 'gaussian', data = rarediv20cm, W = w20cm, 
                          burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                          rho = 1) # rho set to 1 fits an intrinsic CAR prior
hist(residuals(mod.pg20cm))
print(mod.pg20cm) ### DIC = 
plot(mod.pg20cm$samples$beta)

set.seed(32)
mod.pg1m <- S.CARleroux(full.fd1m ~ rare1m, family = 'gaussian', data = rarediv1m, W = w1m, 
                        burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                        rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.pg5m <- MCMCglmm(full.fd5m ~ rare5m, family = 'gaussian', data = rarediv5m,
                     burnin = 10000, nitt = 50000, thin = 1)
plot(mod.pg5m)
summary(mod.pg5m) 
...
set.seed(32)
mod.fdis4cm <- S.CARleroux(fdis4cm ~ rare4cm, family = 'gaussian', data = rarediv4cm, W = w4cm, 
                           burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                           rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.fdis20cm <- S.CARleroux(fdis20cm ~ rare20cm, family = 'gaussian', data = rarediv20cm, W = w20cm, 
                            burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                            rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.fdis1m <- S.CARleroux(fdis1m ~ rare1m, family = 'gaussian', data = rarediv1m, W = w1m, 
                          burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                          rho = 1) # rho set to 1 fits an intrinsic CAR prior
hist(residuals(mod.fdis1m))
print(mod.fdis1m) ## DIC = 
plot(mod.fdis1m$samples$beta)

set.seed(32)
mod.fdis5m <- MCMCglmm(fdis5m ~ rare5m, family = 'gaussian', data = rarediv5m,
                       burnin = 10000, nitt = 50000, thin = 1)
plot(mod.fdis5m)
summary(mod.fdis5m) 
...
set.seed(32)
mod.fdisabund4cm <- S.CARleroux(fdisabund4cm ~ rare4cm, family = 'gaussian', data = rarediv4cm, W = w4cm, 
                                burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                                rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.fdisabund20cm <- S.CARleroux(fdisabund20cm ~ rare20cm, family = 'gaussian', data = rarediv20cm, W = w20cm, 
                                 burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                                 rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.fdisabund1m <- S.CARleroux(fdisabund1m ~ rare1m, family = 'gaussian', data = rarediv1m, W = w1m, 
                               burnin = 10000, n.sample = 50000, prior.tau2 = c(0.5, 0.0005),
                               rho = 1) # rho set to 1 fits an intrinsic CAR prior
...
set.seed(32)
mod.fdisabund5m <- MCMCglmm(fdisabund5m ~ rare5m, family = 'gaussian', data = rarediv5m,
                            burnin = 10000, nitt = 50000, thin = 1)
